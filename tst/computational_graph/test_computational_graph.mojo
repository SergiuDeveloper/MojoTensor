...
fn test_computational_graph_update_weights() raises:
    ...
    fake_w_grad = List[Float64]()
    for i in range(OUTPUT_NEURONS * INPUT_NEURONS):
        grad_val = Float64(i) * 0.01
        fake_w_grad.append(grad_val)
        expected_w.append(w_ptr[i].cast[DType.float64]() - LEARNING_RATE * grad_val)

    fake_b_grad = List[Float64]()
    for i in range(OUTPUT_NEURONS):
        grad_val = Float64(i) * 0.02
        fake_b_grad.append(grad_val)
        expected_b.append(b_ptr[i].cast[DType.float64]() - LEARNING_RATE * grad_val)

    with DeviceContext() as ctx:
        dense_layer.allocate_kernel_memory(ctx, BATCH_SIZE)

        layer_key = computational_graph._compute_layer_key(UnsafePointer(to=dense_layer))

        gradients_data = List[List[Float64]]()
        gradients_data.append(List[Float64]())
        gradients_data.append(List[Float64]())
        for i in range(OUTPUT_NEURONS * INPUT_NEURONS):
            gradients_data[0].append(fake_w_grad[i])
        for i in range(OUTPUT_NEURONS):
            gradients_data[1].append(fake_b_grad[i])

        computational_graph.gradients[layer_key] = gradients_data^
        computational_graph.layer_keys[layer_key] = UnsafePointer(to=dense_layer)

        computational_graph.update_weights()
        ctx.synchronize()

        assert_equal(len(computational_graph.gradients), 0)
        assert_equal(len(computational_graph.layer_keys), 0)
